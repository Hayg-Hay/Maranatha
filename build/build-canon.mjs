...
// chapter). Every translation file validates itself against this.
//
// canon.js intentionally holds NO display names. Book IDs are stable, internal,
// language-independent identifiers (see build-locale.mjs / data/locales/en.json
// for the English names that go with them). This is what lets a future
// Armenian-language translation get added without touching this file.
//
// This is milestone 1 of the Maranatha build, following the same pattern as
...
//
// KNOWN GAPS (both marked `provisional: true` in the output):
...
//      reachable in the session that produced this data, so Baruch is
//      recorded here with only its first 5 chapters. Chapter 6 must be added
//      once the real WEB Catholic Edition (or another verified Catholic
//      source) text is fetched in the milestone-2 translation pipeline.
